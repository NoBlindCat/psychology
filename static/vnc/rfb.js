...
var filterKeyPress = function(keysym) {
    var error_keysyms = [
        65515, 65383 // Fix for Mac Meta key press bug
    ];
    for (var i in error_keysyms) {
        if (keysym == error_keysyms[i]) return true;
    }
    return false;
};
...
keyPress = function(keysym, down) {
    ...
    if (filterKeyPress(keysym)) { return; }
...
that.sendKeysFromClipboard = function(text) {
    var arr = [];
    for(var i = 0; i < text.length; i += 1) {
        var press_shift = needShift(text[i]);
        var key_code = text.charCodeAt(i);
        if(press_shift) {
            arr = arr.concat(keyEvent(0xffe1, 1));
        }
        arr = arr.concat(keyEvent(key_code, 1));
        arr = arr.concat(keyEvent(key_code, 0));
        if(press_shift) {
            arr = arr.concat(keyEvent(0xffe1, 0));
        }
    }
    arr = arr.concat(fbUpdateRequests());
    ws.send(arr);
...
function needShift(character) {
    var need_shift_chars = '~!@#$%^&*()_+{}|:"<>?';
    return need_shift_chars.indexOf(character) >= 0;
};
